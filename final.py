...
from index_pdf import find_first_page, find_index_page, add_link_index
...
def extract_table_contents(pdf_path):
    ...
    generation_config = {
        ...
        "top_p": 0.95,
        "top_k": 40,
        "max_output_tokens": 8192,  # Điều chỉnh giới hạn token
...
def process_pdf(input_pdf, output_pdf, table_contents):
    ...
    if temp_pdf:
        try:
            ...
            index_page, total_page = find_index_page(input_pdf)
            ...
            temp_output = os.path.join(os.path.dirname(output_pdf), "temp_" + os.path.basename(output_pdf))
            if not os.path.exists(temp_output):
                open(temp_output, 'wb').close() 

            add_link_index(output_pdf, index_page, temp_output, k)
...
if __name__ == "__main__":
    pdf_file = "file_pdf/The world's 20 greatest unsolved problems - Vacca, John R - 2005 - Upper Saddle River, NJ Prentice Hall PTR - Anna’s Archive.pdf"
    output_file = "file_pdf/test.pdf"
    table_contents = "table_contents.txt"
    process_pdf(pdf_file, output_file, table_contents)